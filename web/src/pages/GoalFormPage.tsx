// web/src/pages/GoalFormPage.tsx
...
type FormState = {
  content: string
  category?: string
  status: UIStatus     // UIでは 0/1000/999。保存時に 'active'/1000/999 へ変換
  reasonU?: string
}
...
const GoalFormPage: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (!found) { setError('指定された目標が見つかりませんでした'); return }
        setForm({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.content || form.content.trim() === '') { setError('内容を入力してください'); return }
    if ((form.status === 1000 || form.status === 999) && (!form.reasonU || form.reasonU.trim() === '')) {
      setError('達成・中止の理由を入力してください'); return
    }
      ...
      {loading ? (
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input id="content" type="text" className="w-full p-3 border rounded text-lg"
              value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} required />
          </div>

          <div>
            <label className="block mb-1 text-lg" htmlFor="category">カテゴリ（任意）</label>
            <input id="category" type="text" className="w-full p-3 border rounded text-lg"
          ...
          {id && (
            <div>
              ...
              <select id="status" className="w-full p-3 border rounded text-lg"
                value={form.status} onChange={(e) => setForm({ ...form, status: Number(e.target.value) as UIStatus })}>
          ...
          {(form.status === 1000 || form.status === 999) && (
            <div>
              ...
              <textarea id="reason" className="w-full p-3 border rounded text-lg"
                value={form.reasonU || ''} onChange={(e) => setForm({ ...form, reasonU: e.target.value })} rows={3} required />
            </div>
          )}
          ...
          <div className="flex space-x-4">
            <button type="submit" className="flex-1 bg-blue-600 hover:bg-blue-500 text-white text-lg py-3 rounded">保存</button>
            <button type="button" className="flex-1 bg-gray-400 hover:bg-gray-300 text-white text-lg py-3 rounded" onClick={() => navigate('/goals')}>キャンセル</button>
          </div>
        </form>
      )}
    </div>
