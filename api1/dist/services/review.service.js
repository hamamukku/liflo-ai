...
async function loadRows(userId, from, to, goalId) {
...
export async function summary(userId, q) {
    const rows = await loadRows(userId, q.from, q.to, q.goalId);
    const n = rows.length;
    if (n === 0) {
        return {
            count: 0,
    ...
    const mk = (v) => (v > 0 ? `+${v.toFixed(1)}` : v.toFixed(1));
    const notes = `挑戦度の差分(本人-AI)=${mk(biasC)}, 能力度の差分=${mk(biasS)}。` +
