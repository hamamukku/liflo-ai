...
async function _resolveRepos() {
    ...
    if (mem)
        return { goals: mem.goalsMem ?? {}, records: mem.recordsMem ?? {}, users: mem.usersMem ?? {} };
...
export const repos = new Proxy({}, {
    get(_t, repoKey) {
        return new Proxy({}, {
            get(_t2, methodKey) {
                return async (...args) => {
                    const r = await getRepos();
                    const repo = r[repoKey];
                    const fn = repo?.[methodKey];
...
async function _resolveAI() {
    ...
    if (mock?.MockAIProvider)
        return new mock.MockAIProvider();
    return {
    ...
    async append(events) {
        ...
        for (const e of arr) {
            try {
                ...
                if (sev === "ERROR")
                    console.error("[logSink]", line);
                else if (sev === "WARN")
                    console.warn("[logSink]", line);
                else
                    console.log("[logSink]", line);
            }
            catch {
                // ここで落ちないように丸める
