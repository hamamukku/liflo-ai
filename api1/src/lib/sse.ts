...
export function ssePush(
  res: Response,
  data: unknown,
  opts?: { event?: string; id?: string | number }
) {
  const lines: string[] = [];

  if (opts?.id !== undefined) lines.push(`id: ${String(opts.id)}`);
 ...
 */
export function sseOnClose(req: Request, cleanup: () => void) {
  // close は一度だけ
  const once = () => {
    try { cleanup(); } finally { req.off?.('close', once as any); }
  };
  req.on('close', once as any);
}
...
export function createSSE(
  req: Request,
  res: Response,
  opts?: { retry?: number; pad?: boolean; heartbeatMs?: number }
) {
  ...
  return {
    push: (data: unknown, o?: { event?: string; id?: string | number }) =>
      ssePush(res, data, o),
